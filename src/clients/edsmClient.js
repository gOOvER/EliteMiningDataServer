...
class EDSMClient {
  ...
  async getSystemInfo(systemName) {
    try {
      ...
      logger.error(
        `Failed to get system info for ${systemName}:`,
        error.message
      )
      return null
    }
  ...
  async getSystemBodies(systemName) {
    try {
      ...
      logger.error(
        `Failed to get system bodies for ${systemName}:`,
        error.message
      )
      return []
    }
  ...
  async getNearbySystemsWithMaterials(systemName, radius = 50) {
    try {
      ...
      logger.error(
        `Failed to get nearby systems for ${systemName}:`,
        error.message
      )
      return []
    }
  ...
  async findSystemsByName(searchTerm, limit = 10) {
    try {
      ...
      logger.error(
        `Failed to search systems with term "${searchTerm}":`,
        error.message
      )
      return []
    }
  ...
  async getDistanceBetweenSystems(system1, system2) {
    try {
      ...
      logger.error(
        `Failed to calculate distance between ${system1} and ${system2}:`,
        error.message
      )
      return null
    }
  ...
  async getMiningSystemsNearby(systemName, radius = 100) {
    try {
      const nearbySystems = await this.getNearbySystemsWithMaterials(
        systemName,
        radius
      )
      const miningSystems = []
      ...
      for (const system of nearbySystems.slice(0, 50)) {
        ...
        try {
          ...
          const miningBodies = bodies.filter(
            (body) =>
              body.type === 'Belt' ||
          ...
          logger.warn(
            `Could not get bodies for system ${system.name}:`,
            bodyError.message
          )
        }
      }
      ...
      logger.error(
        `Failed to find mining systems near ${systemName}:`,
        error.message
      )
      return []
    }
