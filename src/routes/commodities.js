...
router.get('/mining/overview', async (req, res) => {
  try {
    ...
    const miningCommodities = [
      ...
      'Tritium', 'Platinum', 'Osmium', 'Gold', 'Silver', 'Palladium'
    ]
    ...
    res.status(500).json({ error: 'Failed to fetch mining commodities overview' })
  }
})
...
router.get('/station/:stationName/:systemName', async (req, res) => {
  try {
    ...
    if (commodities.length === 0) {
      return res.status(404).json({ error: 'No market data found for this station' })
    }
