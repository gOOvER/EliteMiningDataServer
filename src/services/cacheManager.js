...
class CacheManager {
  ...
  async initialize() {
    try {
      ...
      if (
        this.config.strategy === 'redis-only' ||
        this.config.strategy === 'hybrid'
      ) {
        try {
          ...
          logger.warn(
            'Redis cache initialization failed, falling back to memory-only:',
            error
          )
      ...
      logger.info(
        `Cache Manager initialized with strategy: ${this.config.strategy}`
      )
  ...
  async set(key, value, options = {}) {
    ...
    try {
      ...
      if (
        this.redisCache &&
        (useStrategy === 'hybrid' || useStrategy === 'redis-only')
      ) {
      ...
      if (
        this.inMemoryCache &&
        (useStrategy === 'hybrid' || useStrategy === 'memory-only')
      ) {
  ...
  cacheAPIResponseTime(endpoint, method, duration) {
    if (this.inMemoryCache) {
      return this.inMemoryCache.cacheAPIResponseTime(
        endpoint,
        method,
        duration
      )
  ...
  async invalidateCache(dataType, trigger, context = {}) {
    if (this.invalidationManager) {
      return await this.invalidationManager.invalidateCache(
        dataType,
        trigger,
        context
      )
